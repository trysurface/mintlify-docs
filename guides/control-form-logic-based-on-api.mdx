...
<Frame>
  <img className="block" src="/images/form-logic-external-api/5.png" alt="Create an Advanced Routing rule using External API Data" />
</Frame>
...
3. Under **IF**, set:
   ...
   - **Condition**: choose the field you mapped (for example, `authenticated`) and compare it to the expected value (for example, `true`)



### Example: Only proceed if authenticated

- **IF**: `External API Data.authenticated` equals `true`
- **THEN**: Jump to `Step: Success`
