...
flowchart TD
    subgraph Surface
        A[Lead submits form partial/completed]
    end

    subgraph CustomerIO
        B[Webhook receives data - partial or completed URL]
        C[Create/Update Person]
        D[Send Event: partialFormFilled or formCompleted]
        E[Event triggers campaign]
        F[Send first follow-up email]
        G[Delay]
        H[Webhook: Check Calendly]
        I{Meeting booked?}
        J[Exit campaign]
        K[Send next follow-up]
    end
