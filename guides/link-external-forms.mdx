...
## Step 1: Update the Surface Tag
### Add `defer` Attribute
To ensure that external HTML forms work seamlessly with Surface Labs, the Surface Tag must be updated to include the `defer` attribute. This ensures that the script loads asynchronously and does not block the rendering of the page.

```html
<!-- Before -->
<script src="https://cdn.jsdelivr.net/gh/trysurface/scripts@latest/surface_tag.min.js">
</script>

<!-- After -->
<script defer src="https://cdn.jsdelivr.net/gh/trysurface/scripts@latest/surface_tag.min.js">
</script>
```

### Add Site ID to Surface Tag
<Frame>
  <img src="/images/external-form/5.png" />
</Frame>

To ensure that the Surface Tag tracks the correct site, the `site-id` attribute must be added to the Surface Tag script.

```html
<script defer site-id="<Your_Site_ID>" src="https://cdn.jsdelivr.net/gh/trysurface/scripts@latest/surface_tag.min.js">
</script>
```
...
<Info>
  If separate forms exist for each step, the `data-id` attribute must be added to each form.
</Info>
...
### Add Hidden Field to Mark Form as Completed
For the final step, include a hidden field before the submit button to mark the form as completed:
...
2. **Partial responses not tracking**:
   - Ensure each step has the proper data-id attributes.
   - Check the network tab to confirm requests are being sent.

3. **Surface Tag not working**:
   - Verify the script has the `defer` attribute.
